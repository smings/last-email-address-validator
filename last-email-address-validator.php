...
Description: LEAV provides the best free email address validation for WP registration/comments, WooCommerce, Contact Form 7, WPForms, Ninja Forms and more plugins to come...
Version: 1.3.0
Author: smings
...
$disposable_email_service_mx_server_domains_file =  plugin_dir_path(__FILE__) . "data/disposable_email_service_provider_mx_server_domain_list.txt";
$disposable_email_service_ip_list_file = =  plugin_dir_path(__FILE__) . "data/disposable_email_service_provider_mx_server_list.txt";

$leav_plugin_website = "https://smings.com/last-email-address-validator/";
$leav_plugin_name = "LEAV - Last Email Address Validator";
$leav_plugin_short_name = "LEAV";
...
function leav_init() 
{
    ...
    if (empty($leav_options["disposable_email_service_ip_list"])) {
        $disposable_email_service_ips = file_exists($disposable_email_service_ip_list_file) ?   file_get_contents($disposable_email_service_ip_list_file) : "";
        $leav_options["disposable_email_service_ip_list"] = $disposable_email_service_ips;
    }
...
{    
    ...
    for( $i = 1; $i <= $size; $i++ )
    {
        if( preg_match( "/^.*e[^a-zA-Z0-9]{0,2}mail.*$/i", $form_data["fields"][$i]["key"] ) == 1   
...
function leav_powered_by_label($string_before = "", $string_after = "")
{
    ...
    $label = $string_before . __("Anti spam protected by", "leav") . ' <a href="' . $leav_plugin_website . '" title="' . $leav_plugin_name . '" target="_blank">'. $leav_plugin_short_name .'</a> - <strong>%s</strong> ' . __("Spam email addresses blocked", "leav") . "!" . $string_after;
